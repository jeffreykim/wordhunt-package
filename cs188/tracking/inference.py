...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for variable in variables[:3]:
        variableDomainsDict[variable] = positions
    for observer in variables[3:]:
        variableDomainsDict[observer] = list(range(X_RANGE+Y_RANGE+MAX_NOISE-1))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            factorsWithoutV = list(filter(lambda f: variable not in f.variablesSet(), factors))
            factorsWithV = list(filter(lambda f: variable in f.variablesSet(), factors))
            j = joinFactorsByVariable(factorsWithV, variable)[1]
            factors = factorsWithoutV
            if len(j.unconditionedVariables()) != 1:
                factors += [eliminate(j, variable)]
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        copy = self.copy()
        copy.normalize()
        for item, val in copy.items():
            ranges[curr] = item
            rangesList.append(curr)
            curr += val
        rangesList.append(1.0)

        left = 0
        right = len(rangesList) - 1

        while left < right:
            mid = (left + right) // 2
            if rangesList[mid] < x < rangesList[mid+1]:
                return ranges[rangesList[mid]]
            if x <= rangesList[mid]:
                right = mid
            else:
                left = mid
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        position is known.
        """
        self.beliefs = DiscreteDistribution({g: self.getObservationProb(observation, gameState.getPacmanPosition(), g, self.getJailPosition()) * self.beliefs[g] for g in self.allPositions})
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for curr in self.allPositions:
            for nextPos in self.getPositionDistribution(gameState, curr):
                beliefs[nextPos] += self.beliefs[curr] * self.getPositionDistribution(gameState, curr)[nextPos]
