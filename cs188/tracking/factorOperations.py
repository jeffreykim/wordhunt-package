...
def joinFactors(factors: List[Factor]):
    ...
    f = Factor(unconditioned, conditioned, variableDomainsDict)

    for a in f.getAllPossibleAssignmentDicts():
        probabilities = [factor.getProbability(a) for factor in factors]
        for i in range(1, len(probabilities)):
            probabilities[i] *= probabilities[i-1]
        f.setProbability(a, probabilities[-1])
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        f = Factor(unconditioned, conditioned, VDD)

        for assign in f.getAllPossibleAssignmentDicts():
            p = [0 for i in range(len(VDD[eliminationVariable])+1)]
            for i in range(len(VDD[eliminationVariable])):
                a = assign.copy()
                a[eliminationVariable] = VDD[eliminationVariable][i]
                p[i+1] = p[i] + factor.getProbability(a)
            f.setProbability(assign, p[-1])
        return f
